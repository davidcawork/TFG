...
function install {
	...
	ip link add dos type veth peer name veth0
	ip link set veth0 netns dos
	
	# Finally assign Ip's, and raise each one of them.
	ip link set uno up
	ip addr add 10.0.0.1/24 dev uno
	ip link set dos up
	ip addr add 10.0.0.3/24 dev dos
	ip netns exec uno ip link set veth0 up
	ip netns exec uno ip addr add 10.0.0.2/24 dev veth0
	ip netns exec dos ip link set veth0 up
	ip netns exec dos ip addr add 10.0.0.4/24 dev veth0
}
